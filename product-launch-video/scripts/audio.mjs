...
//        --language defaults to "en"; --voice overrides voice selection.
//   check — verify the shared engine resolves; run in the FOREGROUND before
//        backgrounding generate (a backgrounded death is invisible until Step 5).
//   sync-durations — write real voice durations into STORYBOARD.md and rescale
//        each synced frame's Scene windows to the real duration (local).
...
import { rescaleSceneLine } from "./lib/rescale-scene-times.mjs";
...
function runGenerate(argv) {
  ...
  const language = flag(argv, "language", "en");
...
function runSyncDurations(argv) {
  ...
  // Tail stays permissive (matches the pre-rescale behavior of replacing the
  // whole value): `- duration: 5.0s (estimated)` still syncs, the note is dropped.
  const DURATION_RE = /^(\s*[-*]\s+duration\s*:\s*)(?:([\d.]+)\s*s?\b)?.*$/i;

  // Pass 1 — per frame, the estimated duration Step 4's Scene windows were
  // written against, so pass 2 can rescale the windows by real/estimated.
  const ratioByFrame = new Map();
  let curFrame = null;
  for (const line of lines) {
    const h = line.match(FRAME_RE);
    if (h) {
      curFrame = Number(h[1]);
      continue;
    }
    if (curFrame == null || !durByFrame.has(curFrame)) continue;
    const m = line.match(DURATION_RE);
    if (m && Number(m[2]) > 0) ratioByFrame.set(curFrame, durByFrame.get(curFrame) / Number(m[2]));
  }

  // Pass 2 — rewrite duration lines to the real voice length and rescale the
  // frame's Scene windows by the same ratio. Silent frames (no voice entry)
  // keep their estimates untouched.
  curFrame = null;
  let updated = 0;
  let rescaled = 0;
  for (let i = 0; i < lines.length; i++) {
    ...
    if (durByFrame.has(curFrame)) {
      const m = lines[i].match(DURATION_RE);
      if (m) {
    ...
    if (ratioByFrame.has(curFrame)) {
      const next = rescaleSceneLine(lines[i], ratioByFrame.get(curFrame));
      if (next !== lines[i]) {
        lines[i] = next;
        rescaled++;
      }
  ...
  console.log(
    `✓ audio sync-durations: ${updated} frame duration(s) updated · ${rescaled} Scene window(s) rescaled` +
      (missing.length ? ` · no \`- duration:\` line for frame(s) ${missing.join(", ")}` : ""),
  );
}

// ── check (foreground preflight) ─────────────────────────────────────────────
function runCheck() {
  const engine = process.env.HF_MEDIA_ENGINE || DEFAULT_ENGINE;
  if (!existsSync(engine)) {
    console.error(
      `✗ audio check: media audio engine not found at ${engine}\n` +
        `  Install the media-use skill next to this one (npx hyperframes init refreshes the set),\n` +
        `  or point $HF_MEDIA_ENGINE at an engine script. Do NOT background generate until this passes.`,
    );
    process.exit(1);
  }
  console.log(`✓ audio check: engine at ${engine}`);
}

// ── dispatch ──────────────────────────────────────────────────────────────────
const sub = process.argv[2];
if (sub === "check") runCheck();
else if (sub === "sync-durations") runSyncDurations(process.argv.slice(3));
