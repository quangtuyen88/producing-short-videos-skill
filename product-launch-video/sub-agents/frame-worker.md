...
- **Word timings** (voiced frames) — the frame's `words[]` from `audio_meta.json`: each spoken word's `text` / `start` / `end` in frame-relative seconds. **This is the sync truth.** The Scene windows were estimated before the voice existed (then proportionally rescaled); when a Scene's window and the timestamp of its spoken cue disagree, place the reveal on the word timestamp. No `words[]` in your context → build from the Scene windows as-is.
...
## Product / feature chrome (when story uses density-and-chrome)

If `## Video direction` or the frame block specifies dark tech explainer / dual-column / bottom summary / cast dual|single:

- Implement **top bar + dual-column stage + bottom summary box** as specified in `../references/density-and-chrome.md`.
- These are intentional designed chrome — **not** the forbidden "browser chrome" ban above.
- Ship CJK `@font-face` files under `assets/fonts/` with **root-relative** URLs when Japanese is on screen.
- Prefer skipping karaoke captions when the bottom summary box is present (avoid double text bands).
- **Cast dock** — when Video direction says `cast: dual` or `cast: single`, implement the small avatar dock per `../references/cast-avatars.md` (`cast dock dual` / `cast dock single`): 88–112px circles, accent ring, nameplates, speaking ring pulse on the active host from `speaker: pip|bolt`. Assets live under `assets/images/pip-avatar.*` / `bolt-avatar.*` / `mascot.*`.
- **Cast / mascot never covers text** — reserved gutter only (stacked-left or corners for small dock; bottom-right for optional full-body); reserve column padding; fail QA if any title/stat/card/summary intersects an avatar (see `../references/cast-avatars.md` + `../references/density-and-chrome.md`).

- Ground stays on a full-duration `class="clip"` background layer.
...
- **Product/feature density** — if Video direction names explainer chrome / dual-column, implement topbar + cards + bottom summary per `../references/density-and-chrome.md`; never ship a single centered line on empty canvas.
- **Cast dock** — if Video direction requires `cast: dual|single`, dock is present per `../references/cast-avatars.md`; speaking host has ring highlight (`.is-speaking`); no text/card/summary cover.
- **Canvas containment (compute, don't eyeball)** — every element's **final-state** bounding box must lie fully inside the canvas (0…width × 0…height), including late-revealing chips/cards and anything positioned with `left: 50% + translateX(offset)` (estimate rendered text width: monospace ≈ 0.62×font-size per char + padding + letter-spacing). Prefer edge-anchoring (`right:`/`left:` with a margin) over center-plus-offset for elements near an edge. Any overflow = fail; re-position and re-check. Deliberate full-bleed backgrounds are the only exception.
- **Dock no-cover geometry (compute, don't eyeball)** — when a dock is present, compute the dock's bounding box (circles + nameplates) and the **final-state** box of every text/card/stat element, including elements whose clip window starts mid-frame (a late-revealing stat card is the classic collision). Any intersection = fail: re-position the content element out of the dock column (per cast-avatars.md the stacked-left strip x 0–≈180px below y≈560 is dock-only; content columns start ≥200px), never move the dock, then re-check.
- **Shot-sequence fidelity** — every Scene in the time-coded sequence is realized as a phase, the blueprint's signature move (unless `compose`) is present and recognizable, and the shot reveals to the voiceover (never front-loaded at `t=0`); with `words[]` in context, each reveal sits on its spoken cue's actual timestamp.
