...
- **Don't show**: real product nav bars / scrollbars stolen from a site, real OS cursors, generic decorative shapes standing in for a real asset, floating bokeh / purple-blue "AI" gradients — unless it's an intentional UI-demo reconstruction.
- **Exception — explainer chrome is required for product/feature density:** a designed **top status bar** and **bottom summary box** are *not* forbidden browser chrome; they are the default high-readability system for AI/model/API/feature videos. See `density-and-chrome.md`.

## Product / feature density (mandatory quality bar)

Read **`density-and-chrome.md`** before writing Scene lines for product launches, feature reveals, model intros, or Japanese 解説 videos.

Hard rules:

1. **No sparse single-line slides.** Each content frame is dual-column (title stack + cards/stats) or multi-card dense layout.
2. **Persistent chrome** when using the dark tech explainer system: top bar + bottom summary box.
3. **Primary visual ≥ 40%** of the canvas; fill the top ~83% region (bottom chrome may own the lower strip when used).
4. **Reveal density to the VO** — cards/stats arrive on spoken cues; never one dump then freeze.
5. **Prefer developing graphics over static cards** — when a beat carries data, structure, or a process, use the dynamic data-viz & flow vocabulary in `density-and-chrome.md` (self-drawing flow/pipeline diagrams, node graphs with connecting edges, sequential bar ladders, count-up metrics); at least one graphic element should still be arriving or connecting past the frame's halfway point.
6. Prefer dark tech explainer for AI/coding/API products over empty light consulting slides.

When writing `## Video direction`, name the density system explicitly (e.g. `dark tech explainer chrome · dual-column · bottom summary`).
...
- Product/feature frames pass the **density bar** in `density-and-chrome.md` (no sparse single-line slides).
