...
| `Recap / 解説 Cascade`     | Event, match, launch recap, or long feature 解説 (EN or JP dual-host) | hook → context/stakes → timeline beats → key moments → proof/numbers → takeaway → CTA/outro |
...
`hook | pain_point | product_intro | feature_showcase | benefit_highlight | social_proof | branding | cta | context | timeline_beat | key_moment | takeaway`

Recap / 解説 Cascade extras: `context` (stakes/setting), `timeline_beat` (ordered event step), `key_moment` (highlight clip/stat), `takeaway` (one-line lesson before CTA).
...
## Dual-cast dialogue (Pip & Bolt)

When Step 0 locked `cast: dual` (default for JP 解説 / dark-tech explainer / recap cascade):

1. Write VO as **short alternating turns** — not monologue walls. Pip hooks and reacts; Bolt delivers facts and numbers.
2. Tag each spoken frame in `SCRIPT.md` / the storyboard with **`speaker: pip`** or **`speaker: bolt`** (see `cast-avatars.md`).
3. **JP:** short clauses, numbers spoken clearly. **EN:** same cue-segmentation rule as the script bank (phrase boundaries for on-screen reveals).
4. Personality: **Pip** = hooks / reactions / energy / CTA; **Bolt** = facts / numbers / timeline / takeaway.
5. Stage cast assets from skill `assets/mascots/` into project `assets/images/` when cast ≠ none (orchestrator Step 2/4).

When `cast: single`, one host (Pip) may still use `speaker: pip` for speaking-state ring pulse. When `cast: none`, omit speaker tags.

## Product / feature density

For product launches, feature reveals, model intros, launch recaps, and Japanese 解説-style videos (including the **Recap / 解説 Cascade** arc and dual-cast dialogue above), plan **information-dense beats** (positioning roles, benchmarks, surfaces, pricing, CTA) — not slogan-only slides. Pair each beat with on-screen cards/stats the VO can cue. Full visual standard → `density-and-chrome.md`; cast dock → `cast-avatars.md`.
...
- Product/feature / 解説 arcs plan dense beats (roles, proof, surfaces, pricing) per `density-and-chrome.md` — not slogan-only slides.
- When `cast: dual`, VO is alternating Pip/Bolt turns with `speaker:` tags; Recap / 解説 Cascade used when the brief is a recap or long feature 解説.
